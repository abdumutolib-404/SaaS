...
export const openRouterService = {
     ...
     */
    async generateResponse(prompt, modelId, userId, user) {
        // Get user's plan
        ...
        if (!rateLimit.allowed && !isPro) {
            ...
            throw new Error(`Juda ko'p so'rov yuborildi! ${waitMinutes} daqiqadan keyin qayta urinib ko'ring.\n\n` +
        ...
        if (model.model_type === 'PRO') {
            if (!isPro) {
                ...
                if (!usageCheck.allowed) {
                    throw new Error(`🔒 Premium model limiti tugagan!\n\n` +
                        `🤖 Model: ${model.name}\n` +
     ...
     */
    async generateWithFreeAPI(prompt, modelId, userId, user) {
        try {
     ...
     */
    async tryGroqAPI(prompt, modelId, userId, user) {
        try {
            ...
            const groqModel = groqModels[modelId] || 'llama3-8b-8192';
     ...
     */
    async tryHuggingFaceAPI(prompt, modelId, userId, user) {
        try {
            const systemMessage = this.createSystemMessage(user);
     ...
     */
    async generateWithOpenRouter(prompt, modelId, userId, user, model, userPlan, isPro, rateLimit) {
        try {
            const systemMessage = this.createSystemMessage(user);

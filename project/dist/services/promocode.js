...
export const promocodeService = {
    ...
    async createPromocode(data) {
        try {
            const { code, type, description, daily_tokens = 0, total_tokens = 0, tts_limit = 0, stt_limit = 0, pro_days = 0, plan_name = '', max_usage, created_by } = data;
            // Validate input
    ...
    async usePromocode(code, userId) {
        try {
            ...
            const alreadyUsed = await database.get('SELECT * FROM promocode_usage WHERE promocode_id = ? AND user_id = ?', [promocode.id, userId]);
            if (alreadyUsed) {
            ...
            // Record usage
            await database.run('INSERT INTO promocode_usage (promocode_id, user_id) VALUES (?, ?)', [promocode.id, userId]);
            // Update usage count
            await database.run('UPDATE promocodes SET current_usage = current_usage + 1 WHERE id = ?', [promocode.id]);
     ...
     */
    async addTTSLimit(userId, limit) {
        try {
     ...
     */
    async addSTTLimit(userId, limit) {
        try {
            const monthYear = new Date().toISOString().substring(0, 7);
            await database.run(`
    ...
    async getPromocodeByCode(code) {
        try {
            return await database.get('SELECT * FROM promocodes WHERE code = ? AND is_active = 1', [code.toUpperCase()]);
        }
        catch (error) {

...
export class TelegramFormatter {
     ...
     */
    static formatWelcomeNew(firstName) {
        return [
     ...
     */
    static formatWelcome(firstName, userId) {
        return [
            `🎉 Assalomu alaykum, ${this.bold(firstName)}!`,
            '',
            '🤖 Men AI chatbot man. Sizga turli AI modellari bilan suhbatlashishda yordam beraman.',
     ...
     */
    static formatReferralWelcome(firstName, userId, referrerName) {
        return [
            `🎉 Assalomu alaykum, ${this.bold(firstName)}!`,
            '',
     ...
     */
    static formatRegistrationStep(step) {
        switch (step) {
            case 'name':
                return [
                    '👤 Ismingizni kiriting:',
     ...
     */
    static formatRegistrationComplete(userId) {
        return [
     ...
     */
    static formatRegistrationSkipped(userId) {
        return [
     ...
     */
    static formatChatModeStart(modelName) {
        return [
     ...
     */
    static formatModelSelected(modelName, modelType = 'FREE') {
     ...
     */
    static formatAvailablePromocodes(promocodes) {
        const lines = [
        ...
        if (promocodes.length === 0) {
            ...
            promocodes.forEach((promo, index) => {
                ...
                lines.push(`   📊 Qolgan: ${remaining}/${promo.max_usage}`, '');
            });
        }
        lines.push(this.bold('Qanday ishlatish:'), this.code('/promocode <kod>'), '', this.bold('Misol:'), this.code('/promocode BONUS2025'));
        return lines.join('\n');
    }
    /**
     * Format image generation result
     */
    static formatImageGenerated(prompt, remaining, limit) {
        return [
     ...
     */
    static formatAdminPromocodeGuide() {
        return [
     ...
     */
    static formatTokenLimit(userId) {
        return [
     ...
     */
    static formatAdminTokenUsage(operation) {
     ...
     */
    static formatHelp(user, isAdmin = false) {
        const lines = [
        ...
        if (isAdmin) {
            lines.push(this.bold('Admin buyruqlari:'), `${this.code('/admin')} - Admin panel`, `${this.code('/add\\_tokens <user\\_id> <daily> <total>')} - Token qo\\'shish`, `${this.code('/remove\\_tokens <user\\_id> <daily> <total>')} - Token ayirish`, `${this.code('/grant\\_pro <user\\_id> <days>')} - PRO berish`, `${this.code('/change\\_plan <user\\_id> <plan>')} - Plan o\\'zgartirish`, `${this.code('/add\\_promo <code> <daily> <total> <usage>')} - Promokod yaratish`, `${this.code('/delete\\_promo <code>')} - Promokod o\\'chirish`, `${this.code('/broadcast <xabar>')} - Xabar yuborish`, '');
        }
        lines.push(this.bold('Qanday foydalanish:'), '• Shaxsiy chatda: "Suhbat boshlash" tugmasini bosing', '• Guruhda: Botga reply qiling yoki @mention qiling', '', this.bold('Rejalar:'), '🆓 FREE: 2,000 kunlik, 15,000 umumiy token', '💎 PRO: 8,000 kunlik, 80,000 umumiy token + Premium modellar (8,000 so\'m/oy)', '🌟 PREMIUM: 12,000 kunlik, 150,000 umumiy token + Barcha imkoniyatlar (20,000 so\'m/oy)', '', this.bold('PRO xususiyatlari:'), '💎 Premium AI modellari (GPT-4, Claude-3, Gemini Pro)', '🔊 Ko\'proq ovoz yaratish (3/oy vs 1/oy)', '🎤 Ko\'proq STT (3/oy vs 1/oy)', '⚡ Tezroq ishlov berish', '💾 Suhbatlarni saqlash (tez orada)', '📤 Eksport qilish (tez orada)', '🧠 Xotira rejimi (tez orada)', '', this.bold('Sizning ma\'lumotlaringiz:'), `🆔 ID: ${this.code(String(user?.telegram_id))}`, `📋 Plan: ${user?.plan_type || 'FREE'}`, `${user?.is_pro ? '💎' : '🆓'} Status: ${user?.is_pro ? 'PRO' : 'FREE'}`, `🔥 Kunlik limit: ${user?.daily_tokens || 0} token`, `💎 Umumiy limit: ${user?.total_tokens || 0} token`, '', this.bold('Token tugasa:'), 'Admin bilan bog\'laning: @abdulahadov\\_abdumutolib');
        return lines.join('\n');
    }
    /**
     * Create formatted promocode success message
     */
    static formatPromocodeSuccess(message, dailyTokens, totalTokens) {
        return [
     ...
     */
    static formatTokenOperation(userId, dailyTokens, totalTokens, operation) {
    ...
    static formatReferralStats(stats) {
        ...
        if (stats.recent_referrals && stats.recent_referrals.length > 0) {
            ...
            stats.recent_referrals.slice(0, 5).forEach((referral, index) => {
        ...
        lines.push(this.bold('💡 Qanday ishlaydi:'), '• Havolangizni do\'stlaringizga yuboring', '• Ular bot orqali ro\'yxatdan o\'tganda:', '  - Siz: +5,000 kunlik, +15,000 umumiy token', '  - Ular: +3,000 kunlik, +10,000 umumiy token', '', this.bold('🎯 Bonus takliflar:'), '• 5 taklif = 25,000 token bonus', '• 10 taklif = 50,000 token bonus', '• 25 taklif = 150,000 token bonus');
        return lines.join('\n');
    }
    /**
     * Format referral notification for referrer
     */
    static formatReferralNotification(referredName) {
        return [
     ...
     */
    static formatAdminReferralStats(stats) {
        const lines = [
            this.bold('👥 Taklif tizimi statistikasi:'),
            '',
        ...
        if (stats.top_referrers && stats.top_referrers.length > 0) {
            ...
            stats.top_referrers.forEach((user, index) => {
    ...
    static formatProStats(stats) {
        ...
        if (stats.is_pro) {
            lines.push(this.bold('💎 PRO Status:'), `⏰ Amal qiladi: ${stats.pro_expires_at ? new Date(stats.pro_expires_at).toLocaleDateString('uz-UZ') : 'Cheksiz'}`, '');
            if (stats.pro_models && stats.pro_models.length > 0) {
                lines.push(this.bold('🤖 PRO Modellar ishlatilishi:'));
                stats.pro_models.forEach((model) => {
                    lines.push(`• ${model.name}: ${model.usage_count}/${model.monthly_limit}`);
                });
                lines.push('');
            }
        }
        lines.push(this.bold('🔊 Ovoz yaratish:'), `📊 Ishlatilgan: ${stats.tts_usage}/${stats.tts_limit}`, `🔄 Qolgan: ${stats.tts_limit - stats.tts_usage}`, '', this.bold('🎤 Ovozni matnga:'), `📊 Ishlatilgan: ${stats.stt_usage}/${stats.stt_limit}`, `🔄 Qolgan: ${stats.stt_limit - stats.stt_usage}`, '');
        if (!stats.is_pro) {
            lines.push(this.bold('💎 PRO ga o\'tish:'), '• Premium AI modellari', '• Ko\'proq ovoz yaratish (3/oy)', '• Ko\'proq STT (3/oy)', '• Tezroq ishlov berish', '• Suhbatlarni saqlash (tez orada)', '• Eksport qilish (tez orada)', '• Xotira rejimi (tez orada)', '', 'Admin bilan bog\'laning: @abdumutolib\\_abdulahadov');
        }
        return lines.join('\n');
    }
    /**
     ...
     */
    static formatPlansComparison(comparison) {
        const lines = [
        ...
        comparison.plans.forEach((plan) => {
            ...
            lines.push(`${emoji} ${this.bold(plan.display_name)}`, plan.price_monthly > 0 ? `💰 ${plan.price_monthly} so'm/oy` : '💰 Bepul', '');
            plan.features.forEach((feature) => {
                lines.push(`  ✓ ${feature}`);
            });
            lines.push('');
        });
        lines.push(this.bold('📞 Plan o\'zgartirish:'), 'Admin bilan bog\'laning: @abdumutolib\\_abdulahadov');
        return lines.join('\n');
    }
    /**
     * Format plan change notification
     */
    static formatPlanChanged(planName, displayName) {
     ...
     */
    static formatTTSGenerated(remaining, limit) {
        return [
     ...
     */
    static formatSTTConverted(text, remaining, limit) {
        return [
     ...
     */
    static formatProModelLimit(modelName, remaining, limit) {
        return [

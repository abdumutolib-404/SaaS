...
import { coquiTTSService as ttsService } from './services/tts_coqui.js';
import { voskSTTService as sttService } from './services/stt_vosk.js';
import { imageService } from './services/image.js';
import { chatFeaturesService } from './services/chat_features.js';
import { smartFeaturesService } from './services/smart_features.js';
import { freeAPIsService } from './services/free_apis.js';
import { TelegramFormatter } from './utils/formatter.js';
import { KeyboardBuilder, validateKeyboard } from './utils/keyboard.js';
import { logger } from './utils/logger.js';
import { BotContext, User, Model } from './types/bot.js';
...
// Chat mode storage
const chatModes = new Map<number, boolean>();
...
bot.start(async (ctx) => {
  try {
          ...
          if (referrer) {
            ...
            try {
              await bot.telegram.sendMessage(
                referrerId,
                TelegramFormatter.formatReferralNotification(user.first_name)
              );
            } catch (error) {
            ...
            await safeReply(
              ctx,
              TelegramFormatter.formatReferralWelcome(user.first_name, user.telegram_id, referrer.first_name),
              KeyboardBuilder.createRegistrationMenu()
            );
            return;
          }
        }
    ...
    if (!user.registration_completed) {
      await safeReply(
        ctx,
        TelegramFormatter.formatWelcomeNew(user.first_name),
        KeyboardBuilder.createRegistrationMenu()
      );
    } else {
      await safeReply(
        ctx,
        TelegramFormatter.formatWelcome(user.first_name, user.telegram_id),
        KeyboardBuilder.createMainMenu(isAdmin(user.telegram_id), userPlan?.name || 'FREE')
      );
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
...
bot.command('model', async (ctx) => {
  try {
    ...
    await safeReply(ctx, 
      `🤖 ${TelegramFormatter.bold('AI Model tanlang:')}\n\n` +
...
bot.command('promocode', async (ctx) => {
  try {
    ...
    if (result.success && result.tokens) {
      await safeReply(ctx, TelegramFormatter.formatPromocodeSuccess(
        result.message, 
        result.tokens.daily, 
        result.tokens.total
      ));
...
bot.command('tts', async (ctx) => {
  try {
    ...
    if (args.length === 0) {
      await safeReply(ctx, 
        TelegramFormatter.formatError('Format: /tts <matn>') + '\n\n' +
        TelegramFormatter.bold('Misol:') + '\n' +
...
bot.command('grant_pro', async (ctx) => {
  ...
  try {
    ...
    if (args.length < 1 || args.length > 2) {
      await safeReply(ctx, 
        TelegramFormatter.formatError('Format: /grant_pro <user_id> [days]') + '\n\n' +
        TelegramFormatter.bold('Misol:') + '\n' +
    ...
    await safeReply(ctx, TelegramFormatter.formatSuccess(
      `Foydalanuvchi ${userId} ga ${days} kunlik PRO status berildi!`
    ));
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
...
bot.command('change_plan', async (ctx) => {
  ...
  try {
    ...
    if (args.length !== 2) {
      await safeReply(ctx, 
        TelegramFormatter.formatError('Format: /change_plan <user_id> <plan>') + '\n\n' +
        TelegramFormatter.bold('Misol:') + '\n' +
    ...
    if (result.success) {
      ...
      try {
        ...
        if (plan) {
          await bot.telegram.sendMessage(
            userId,
            TelegramFormatter.formatPlanChanged(plan.name, plan.display_name)
          );
        }
      } catch (notifyError) {
...
bot.command('delete_promo', async (ctx) => {
  ...
  try {
    ...
    if (args.length !== 1) {
      await safeReply(ctx, 
        TelegramFormatter.formatError('Format: /delete_promo <code>') + '\n\n' +
        TelegramFormatter.bold('Misol:') + '\n' +
...
bot.command('broadcast', async (ctx) => {
  ...
  try {
    ...
    if (args.length === 0) {
      await safeReply(ctx, 
        TelegramFormatter.formatError('Format: /broadcast <xabar>') + '\n\n' +
        TelegramFormatter.bold('Misol:') + '\n' +
  ...
  try {
    ...
    switch (data) {
      ...
      case 'skip_registration':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.formatRegistrationSkipped(user.telegram_id),
          KeyboardBuilder.createMainMenu(isAdmin(user.telegram_id), userPlan?.name || 'FREE')
        );
        break;
      ...
      case 'start_chat':
        if (!user.selected_model) {
          await safeEdit(ctx, 
            TelegramFormatter.formatError('Avval AI model tanlang!'),
        ...
        await safeEdit(ctx, 
          TelegramFormatter.formatChatModeStart(model?.name || 'Noma\'lum'),
          new KeyboardBuilder().addButton('🔚 Suhbatni tugatish', 'end_chat').build()
        );
        break;
      ...
      case 'end_chat':
        chatModes.delete(user.telegram_id);
        await safeEdit(ctx, 
          TelegramFormatter.formatChatModeEnd(),
          KeyboardBuilder.createMainMenu(isAdmin(user.telegram_id), userPlan?.name || 'FREE')
        );
        break;
      ...
      case 'admin_add_tokens':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.formatAdminTokenUsage('add') + '\n\n' +
          TelegramFormatter.bold('Buyruq:') + '\n' +
          TelegramFormatter.code('/add\\_tokens <user\\_id> <daily> <total>'),
          KeyboardBuilder.createBackButton()
        );
        break;
      ...
      case 'admin_remove_tokens':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.formatAdminTokenUsage('remove') + '\n\n' +
          TelegramFormatter.bold('Buyruq:') + '\n' +
          TelegramFormatter.code('/remove\\_tokens <user\\_id> <daily> <total>'),
          KeyboardBuilder.createBackButton()
        );
        break;
      ...
      case 'admin_grant_pro':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.bold('PRO status berish:') + '\n\n' +
          TelegramFormatter.code('/grant\\_pro <user\\_id> [days]') + '\n\n' +
          TelegramFormatter.bold('Misol:') + '\n' +
          TelegramFormatter.code('/grant\\_pro 123456789 30'),
          KeyboardBuilder.createBackButton()
        );
        break;
      ...
      case 'admin_promocodes':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.bold('Promokod yaratish:') + '\n\n' +
          ...
          TelegramFormatter.code('/delete\\_promo <code>'),
          KeyboardBuilder.createBackButton()
        );
        break;
      ...
      case 'admin_broadcast':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.bold('Xabar yuborish:') + '\n\n' +
      ...
      case 'admin_models':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.bold('Model statistikasi:') + '\n\n' +
          `🆓 FREE modellar: ${freeCount}\n` +
          `💎 PRO modellar: ${proCount}\n` +
          `📊 Jami modellar: ${allModels.length}`,
          KeyboardBuilder.createBackButton()
        );
        break;
      ...
      case 'admin_commands':
        ...
        await safeEdit(ctx, 
          TelegramFormatter.bold('Admin buyruqlari:') + '\n\n' +
          ...
          TelegramFormatter.code('/broadcast <xabar>') + ' - Xabar yuborish',
          KeyboardBuilder.createBackButton()
        );
        break;
        ...
        if (data.startsWith('sel_')) {
          ...
          if (selectedModel.model_type === 'PRO' && !userPlan?.pro_model_access) {
            ...
            if (!usageCheck.allowed) {
              await ctx.answerCbQuery(
                `Bu PRO model! FREE foydalanuvchilar uchun ${usageCheck.limit} marta/oy. Limitingiz tugagan.`, 
                { show_alert: true }
              );
              break;
            }
          ...
          await safeEdit(ctx, 
            TelegramFormatter.formatModelSelected(selectedModel.name, selectedModel.model_type),
            KeyboardBuilder.createMainMenu(isAdmin(user.telegram_id), userPlan?.name || 'FREE')
          );
    ...
    if (session.step) {
      switch (session.step) {
        ...
        case 'interests':
          ...
          const userPlan = await planService.getUserPlan(user.telegram_id);
          await safeReply(ctx, 
            TelegramFormatter.formatRegistrationComplete(user.telegram_id),
            KeyboardBuilder.createMainMenu(isAdmin(user.telegram_id), userPlan?.name || 'FREE')
          );
          break;

...
class DatabaseManager {
  ...
  private init() {
    ...
    try {
      ...
      const userTableInfo = this.db.pragma('table_info(users)') as any[];
      const hasPlanType = userTableInfo.some((col: any) => col.name === 'plan_type');
      ...
      const modelTableInfo = this.db.pragma('table_info(models)') as any[];
      const hasModelType = modelTableInfo.some((col: any) => col.name === 'model_type');
      const hasMonthlyLimit = modelTableInfo.some((col: any) => col.name === 'monthly_limit');
      const hasCategory = modelTableInfo.some((col: any) => col.name === 'category');

...
export const referralService = {
   ...
   */
  async createReferralLink(userId: number, botUsername: string): Promise<ReferralLink> {
    try {
      // Check if user already has a referral link
      let referralLink = database.get(
   ...
   */
  async generateReferralLink(userId: number, botUsername: string): Promise<string> {
  ...
  extractReferrerId(referralCode: string): number | null {
    try {
      ...
      const referralLink = database.get(
        'SELECT user_id FROM referral_links WHERE referral_code = ?',
        [referralCode]
      );
      ...
      if (referralLink) {
        ...
        database.run(
          'UPDATE referral_links SET clicks = clicks + 1 WHERE referral_code = ?',
          [referralCode]
        );
   ...
   */
  async processReferral(referrerId: number, newUserId: number): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const existingReferral = database.get(
   ...
   */
  async getReferrerInfo(userId: number): Promise<any | null> {
    try {
      return database.get(`
        SELECT 
          u.telegram_id,
   ...
   */
  async getSystemReferralStats(): Promise<any> {
    try {

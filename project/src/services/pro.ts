...
export const proService = {
   ...
   */
  async grantProStatus(userId: number, durationDays: number = 30): Promise<void> {
    try {
   ...
   * Expire PRO status
   */
  async expireProStatus(userId: number): Promise<void> {
    try {
      database.run(`
        UPDATE users SET 
          is_pro = 0, 
   ...
   */
  async checkProModelUsage(userId: number, modelId: string): Promise<{ allowed: boolean; remaining: number; limit: number }> {
    try {
      // Get model info
      ...
      if (userPlan?.name === 'PRO') limit = 150;
      else if (userPlan?.name === 'PREMIUM') limit = 999999; // Unlimited
   ...
   */
  async incrementProModelUsage(userId: number, modelId: string): Promise<void> {
    try {
      const userPlan = await planService.getUserPlan(userId);
      const timeKey = userPlan?.name === 'FREE' ? this.getCurrentDay() : this.getCurrentMonthYear();
   ...
   */
  async checkTTSUsage(userId: number): Promise<{ allowed: boolean; remaining: number; limit: number }> {
    try {
      const userPlan = await planService.getUserPlan(userId);
      
      let limit = 1; // FREE default
      if (userPlan?.name === 'PRO') limit = 3;
      else if (userPlan?.name === 'PREMIUM') limit = 10;
   ...
   * Increment TTS usage
   */
  async incrementTTSUsage(userId: number): Promise<void> {
    try {
      const monthYear = this.getCurrentMonthYear();
   ...
   */
  async checkSTTUsage(userId: number): Promise<{ allowed: boolean; remaining: number; limit: number }> {
    try {
      const userPlan = await planService.getUserPlan(userId);
   ...
   * Increment STT usage
   */
  async incrementSTTUsage(userId: number): Promise<void> {
    try {
      const monthYear = this.getCurrentMonthYear();
   ...
   */
  async getProStats(userId: number): Promise<any> {
    try {
      const user = await userService.getUser(userId);

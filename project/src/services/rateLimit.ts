...
export class RateLimitService {
  ...
  async checkRateLimit(userId: number): Promise<{ allowed: boolean; remainingRequests: number; resetTime?: Date }> {
    try {
      const now = new Date();
      const windowStart = new Date(now.getTime() - this.windowSizeMs);

      // Get current rate limit record
      const rateLimit = database.get(
  ...
  getRateLimitStatus(userId: number): { requestCount: number; maxRequests: number; windowStart: Date | null } {
    try {
      const rateLimit = database.get(
        'SELECT * FROM rate_limits WHERE user_id = ?',
        [userId]

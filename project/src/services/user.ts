...
export const userService = {
  ...
  async updateSelectedModel(telegramId: number, modelId: string): Promise<void> {
    try {
      database.run(
  ...
  async updateUserInfo(telegramId: number, info: { name?: string; age?: number; interests?: string }): Promise<void> {
    try {
      const fields = [];
      const values = [];
  ...
  async addTokens(telegramId: number, dailyTokens: number, totalTokens: number): Promise<void> {
    try {
      const user = await this.getUser(telegramId);
      if (!user) {
  ...
  async removeTokens(telegramId: number, dailyTokens: number, totalTokens: number): Promise<{ success: boolean; message: string; currentTokens?: { daily: number; total: number } }> {
    try {
      const user = await this.getUser(telegramId);
      if (!user) {

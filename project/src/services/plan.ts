...
export const planService = {
   ...
   * Get plan by name
   */
  async getPlanByName(planName: string): Promise<Plan | null> {
    try {
   ...
   */
  async getUserPlan(userId: number): Promise<Plan | null> {
    try {
   ...
   * Change user's plan
   */
  async changeUserPlan(userId: number, planName: string, adminId?: number): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  async canAccessProModels(userId: number): Promise<boolean> {
    try {
  ...
  async getPlanStatistics(): Promise<any> {
    try {
      ...
      return {
        ...
        revenue_estimate: stats.reduce((sum: number, stat: any) => {

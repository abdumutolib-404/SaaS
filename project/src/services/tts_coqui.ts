...
export const coquiTTSService = {
   ...
   */
  async generateSpeech(text: string, userId: number, language: string = 'en'): Promise<TTSResponse> {
    try {
      // Check usage limit
   ...
   */
  async generateWithCoquiTTS(text: string, language: string = 'en'): Promise<Buffer | null> {
...
try:
   ...
   */
  async generateWithESpeak(text: string, language: string = 'en'): Promise<Buffer | null> {
    return new Promise((resolve) => {
      try {
        ...
        // Select voice based on language
        let voice = 'en+f3';
        if (language === 'uz') voice = 'en+f3'; // Use English for Uzbek
        if (language === 'ru') voice = 'ru+f3';
